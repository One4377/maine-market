...
import { ReactComponent as GroupIcon } from "./images/delta_sort.svg"
import {
  useTable,
  useFlexLayout,
  useSortBy,
          ...
          <div className="row" {...headerGroup.getHeaderGroupProps()}>
            {headerGroup.headers.map((column) => (
              <div
                ...
                {...column.getHeaderProps([{ style: column.style }, getHeaderProps(column)])}
              >
                {column.canGroupBy ? (
              ...
              >
                {row.cells.map((cell) => (
                  <div
                    ...
                    {...cell.getCellProps([{ style: cell.column.style }, getCellProps(cell)])}
                  >
                    {cell.isGrouped ? (
                      // If it's a grouped cell, add an expander and row count
                      <span
                        className={cx("expanded", { isExpanded: row.isExpanded })}
                        {...row.getToggleRowExpandedProps()}
                      >
                        <LeftArrowIcon />
                        {cell.render("Cell")}{" "}
                        <span className="sub-rows-length">({row.subRows.length})</span>
                      </span>
                    ) : cell.isAggregated ? (
